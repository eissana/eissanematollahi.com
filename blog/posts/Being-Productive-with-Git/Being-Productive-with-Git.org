...
1. ~git diff master dev~ command means get all the changes between ~master~ and ~dev~; see Figure [[diff-AB]]. It is commutative, i.e, ~git diff dev master~ generates exact the same output.
